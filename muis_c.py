    ...
    try:
        while True:
            ...
            if response['success'] == True:
				...
				switch VOICE_COM:
					case "system on":
					case "turn on system":main_screen.system_on();
									      break;
					case "system off":
					case "turn off system":main_screen.system_off();
									  break;
					case "volume up":
					case "increase volume":main_screen.volume_up();
									      break;
                    case "volume down":
					case "decrease volume":main_screen.volume_down();
									      break; 																
					default:
...
def main():
	print("Hello, please say the command to execute:")
...
def isRoomOn(roomname):
    ...
    query = ("select button_pressed from uisipi_usage where roomname LIKE %s and button_pressed IN ('ON', 'OFF', 'REBOOT-SCHEDULED') order by date_of_press DESC LIMIT 1")
    args = (roomname)
...
def projector_config():
    control._write(rb's', rb'2', rb'c',
                   PROJECTOR_SERIAL_CONFIG.encode('utf-8'))
